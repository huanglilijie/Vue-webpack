      ...
      submit () {
        ...
        .then((response) => {
          ...
          if (ordersNumber >= Golab.activequota) {
            ...
            this.$http.post(Config.API_ROOT + 'ecommerce/customers/' + window.localStorage.getItem('uid') + '/orders/' + this.pageParam.orderId + '/payment', {subject: '支付意向金', body: this.pageParam.carName, limit_pay: this.paytype, total_fee: this.pageParam.carIntentionFee, return_url: 'http://192.168.6.122:9000/api/ecommerce/customers/payment/callback?info=test', dealer_code: this.pageParam.dealerCode}).then((response) => {
